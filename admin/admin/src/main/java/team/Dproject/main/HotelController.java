...
public class HotelController {
	...
	@Autowired
	private HotelMapper hotelMapper2;
	
	@Autowired
	private RoomMapper_sks roomMapper;
	
	@Autowired
	private RoomMapper roomMapper2;
	...
	*/
	...
	//예약하기 버튼 클릭 후 로그인 창 열기
		...
		*/
		...
		public String hotel_resvselect(HttpServletRequest req){
			List<hotelDTO> list2=hotelMapper2.listHotel2();
			
			for(hotelDTO dto : list2) {
	
				if(roomMapper2.getRoom2(dto.getHotel_no())!=null){
				String a=roomMapper2.getRoom2(dto.getHotel_no());
				dto.setHp1(a);
				}else{
					dto.setHp1("방 없음");
				}
		...
		public String hotel_resvCancelok(HttpServletRequest req){
			...
			if(user_point<0){
				String msg = "올바르지 않은 예약 취소입니다. 다시 확인 바랍니다.";
