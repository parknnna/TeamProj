...
<!-- Content -->
	...
	<div class="buttonContainer" style="margin-bottom:10px;">
			<button type="submit" class=".btn-outline-primary" style="margin-right:20px;"><i class="fas fa-chevron-left" style="font-size:1.5em;"></i></button>
			<button type="reset" class=".btn-outline-primary" style="margin-right:20px;"><i class="fas fa-bars" style="font-size:1.5em;"></i></button>
			<button type="back" class=".btn-outline-primary" style="margin-right:20px;"><i class="fas fa-chevron-right" style="font-size:1.5em;"></i></button>
	</div>	
	
	<div class="commentContainer" style="width:60%;"> 
		<label for="content" style="float:left;"><i class="far fa-comments"></i>  comment</label>
        <form name="commentInsertForm" action="comment_write.do" method="post">
            <div class="input-group">
               ...
               <%if(member!=null){%>
               <input id="writer"type="hidden" name="writer" value="<%=member.getMember_no()%>"/><%} %>
               
               <%if(member==null){ %>
               <input type="text" class="form-control" id="content" name="content" placeholder="로그인 후 이용이 가능합니다." readOnly><%} %>
               
               <%if(member!=null){ %>
               <input type="text" class="form-control" id="content" name="content" placeholder="내용을 입력하세요."><%} %>
               
               <span class="input-group-btn">
                    <input type="button" class="btn btn-default" name="commentInsertBtn" style="margin-left:15px; margin-top:5px;" onclick="javascript:check(<%=member_no %>)" value="등록">
               </span>
              </div>
        </form>
    </div>
